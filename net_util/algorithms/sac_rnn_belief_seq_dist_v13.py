...
@register_policy_cfg
@dataclass
class SACRNNBeliefSeqDistV13_Config:
    batch_size: int = 256
...
@register_policy
class SACRNNBeliefSeqDistV13(PolicyBase):
    def __init__(
        self,
        cmd_cls: Any,
        cfg: SACRNNBeliefSeqDistV13_Config,
        ...
        self.belief_loss_scale: float = 3.0
    ...
    def _belief_loss(
        ...
        logits_norm = logits.norm(p = 2, dim = -1, keepdim=True)
        tau = self.belief_loss_scale  # Upper bound for norm
        delta = 1 / tau  # Use tau as scale (or make delta a separate hyperparameter)
        clipped_logits = th.where(
            logits_norm >= tau,
            logits * (delta / logits_norm),  # Preserve direction, scale norm to delta
            logits
        )
        
        cross_entropy = F.cross_entropy(clipped_logits, target, reduction="mean")
