...
@register_policy_cfg
@dataclass
class SACRNNBeliefSeqDistV10_Config:
    batch_size: int = 256
...
@register_policy
class SACRNNBeliefSeqDistV10(PolicyBase):
    def __init__(
        self,
        cmd_cls: Any,
    ...
    def _log_batch_stats(self, writer: SummaryWriter, step: int, **kwargs) -> None:
        ...
        # if "mu_region_stats" in kwargs:
        #     mu_region_stats = kwargs["mu_region_stats"]
        #     for region_val, (mu_mean_r, mu_std_r) in mu_region_stats.items():
        #         for i in range(mu_mean_r.shape[0]):
        #             writer.add_scalar(
        #                 f"policy/mu_mean_interf{region_val}_dim{i}",
        #                 mu_mean_r[i].item(),
        #                 step,
        #             )
        #             writer.add_scalar(
        #                 f"policy/mu_std_interf{region_val}_dim{i}",
        #                 mu_std_r[i].item(),
        #                 step,
        #             )
    ...
    def _belief_loss(
        ...
        with th.no_grad():
            belief = (
                F.softmax(y_hat_TBK, dim=-1)
                * th.arange(y_hat_TBK.shape[-1], device=self.device)
    ...
    def _probe_z_ablation_feature(
        ...
        feat_full_TBH, _ = self.net.actor_feature_compute(obs_TBD, z_BH, f_h_burn)

        z_zero_BH = th.zeros_like(z_BH)
        feat_noz_TBH, _ = self.net.actor_feature_compute(obs_TBD, z_zero_BH, f_h_burn)
    ...
    def _probe_z_ablation_action(
        ...
        if noise_std > 0.0:
            ...
            feat_noise_TBH, _ = self.net.actor_feature_compute(obs_TBD, z_noise_BH, f_h_burn)
            a_noise_TBA, _ = self.net.action_compute(feat_noise_TBH, is_evaluate=True)

            delta_noise_TBA = a_full_TBA - a_noise_TBA
            imp_noise_TB = delta_noise_TBA.pow(2).sum(dim=-1).sqrt()
            ...
            writer.add_scalar(
                f"z_importance/act_delta_l2_noise_{tag_suffix}",
                imp_noise_scalar,
                step,
            )
            writer.add_scalar(
                f"z_importance/act_rel_delta_noise_{tag_suffix}",
                rel_imp_noise,
                step,
            )

            writer.add_histogram(
                f"z_importance/act_delta_l2_noise_{tag_suffix}_hist",
                imp_noise_TB.detach().cpu().view(-1),
                step,
            )
    ...
    def train_per_epoch(
        ...
        with th.no_grad():
            ...
            _, f_h_burn_critic = self.net.critic_feature_compute(obs_burn, z_burn.detach())
            _, f_h_burn_actor = self.net.actor_feature_compute(obs_burn, z_burn.detach())
        ...
        feat_actor_TBH, _ = self.net.actor_feature_compute(obs_train, z_BH.detach(), f_h_burn_actor)
    ...
    def tf_act(
        ...
        feat, _eval_h = self.net.actor_feature_compute(obs, z_BH, self._eval_h)
