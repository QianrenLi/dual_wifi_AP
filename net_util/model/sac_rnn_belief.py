...
class Network(nn.Module):
    def __init__(self, obs_dim: int, act_dim: int, hidden=128, belief_dim = 0,
        ...
        # Belief
        self.belief_predict = nn.Sequential(FeatureExtractor(obs_dim, hidden), nn.Linear(belief_dim))
    ...
    def encode(self, obs: th.Tensor, h: th.Tensor): 
        with th.no_grad():
            belief = self.belief_predict(obs)
