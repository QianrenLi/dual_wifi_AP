...
class FeatureExtractor(nn.Module):
    def __init__(self, obs_dim: int, hidden: int, rnn_k: int, rnn_hidden: int):
        super().__init__()
        assert 1 <= rnn_k <= obs_dim
        self.rnn_k, self.rnn_hidden = rnn_k, rnn_hidden
        self.mlp  = nn.Sequential(nn.Linear(obs_dim, hidden), nn.LayerNorm(hidden), nn.GELU())
        self.gru  = nn.GRUCell(rnn_k, rnn_hidden)
        self.fuse = nn.Sequential(nn.Linear(hidden + rnn_hidden, hidden), nn.LayerNorm(hidden), nn.GELU())
    ...
    def forward(self, obs: th.Tensor, h: th.Tensor) -> Tuple[th.Tensor, th.Tensor]:
        mlp = self.mlp(obs); h1 = self.gru(obs[..., -self.rnn_k:], h)
        return self.fuse(th.cat([mlp, h1], -1)), h1
...
class Network(nn.Module):
    def __init__(self, obs_dim: int, act_dim: int, hidden=128, rnn_k=8, rnn_hidden=64,
        ...
        self.fe = FeatureExtractor(obs_dim, hidden, rnn_k, rnn_hidden)
        ...
        qin = hidden + act_dim
        make_q = lambda: nn.Sequential(nn.Linear(qin, hidden), nn.LayerNorm(hidden), nn.GELU(), nn.Linear(hidden, 1))
