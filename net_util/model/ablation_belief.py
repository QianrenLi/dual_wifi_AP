## In this model, the critic and actor do not use GRU or belief states.
## Ablated version: belief module removed for comparison studies.
...
class Network(nn.Module):
    ...
    def critic_compute(self, obs: th.Tensor, a: th.Tensor) -> th.Tensor:
        # Critic takes [obs, action] as input (no belief states)
        x = th.cat([obs, a], dim=-1)
        zs = []
    ...
    def target_backup_seq(
        ...
        # Actor takes obs directly (no belief states)
        a_TBA, logp_TB1 = self.action_compute(nxt_TBD, is_evaluate=False)

        # Critic takes [obs, action] as input
        z_list = []
        for qt in self.critics_t:
            z_list.append(qt(th.cat([nxt_TBD, a_TBA], dim=-1)).unsqueeze(2))
