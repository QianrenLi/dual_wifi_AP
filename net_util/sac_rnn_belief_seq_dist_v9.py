...
class SACRNNBeliefSeqDistV9(PolicyBase):
    ...
    def __init__(self, cmd_cls: Any, cfg: SACRNNBeliefSeqDistV9_Config, rollout_buffer: RNNPriReplayBuffer2 | None = None, device: str | None = None, state_transform_dict: dict | None = None, reward_cfg: Optional[dict] = None):
    ...
    def _probe_z_ablation_action(
        ...
        # 1) Get z from belief encoder (no grad, just a probe)
        z_BH, _, _, _ = self.net.belief_encode(obs_TBD, b_h=h_burn, is_evaluate=True)      # [T,B,H_z] or [B,H_z] depending on impl
    ...
    def train_per_epoch(
        ...
        ## Alpha & Critic & Actor
        z_BH, _, mu_BH, logvar_BH = self.net.belief_encode(obs_train, b_h=h_burn, is_evaluate=True)
