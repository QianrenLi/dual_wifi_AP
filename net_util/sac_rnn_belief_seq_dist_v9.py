...
@register_policy_cfg
@dataclass
class SACRNNBeliefSeqDistV9_Config:
    batch_size: int = 256
...
@register_policy
class SACRNNBeliefSeqDistV9(PolicyBase):
    def __init__(
        self,
        cmd_cls: Any,
        cfg: SACRNNBeliefSeqDistV9_Config,
    ...
    def _critic_loss(
        ...
        q1_pi, q2_pi = self.net.critic_compute(feat_TBH, act_TBA)
        log_q1 = (q1_pi.clamp_min(1e-8)).log()
        log_q2 = (q2_pi.clamp_min(1e-8)).log()
        
        kl_loss =  - 0.5 *( (backup_q_TB1 * (log_q1)).sum(dim=-1) + (backup_q_TB1 * (log_q2)).sum(dim=-1))
        c_loss_batch = kl_loss.mean(0)
    ...
    def _actor_loss(self, feat_TBH: Tensor, a_pi_TBA: Tensor, logp_TB1: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        q_val = th.min(
            self.vd.mean_value(q1_pi),
            self.vd.mean_value(q2_pi),
        )
    ...
    def _probe_z_ablation_feature(
        ...
        feat_full_TBH, _ = self.net.feature_compute(obs_TBD, z_BH, f_h_burn)
    ...
    def train_per_epoch(
        ...
        if self._global_step % (self.cfg.log_interval * 10) == 0:
            ...
            self._probe_z_ablation_action(
                obs_train, h_burn, f_h_burn, writer, self._global_step, noise_std=0.1
            )
