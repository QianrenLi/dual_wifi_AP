...
class SACRNN_Config:
    ...
    obs_dim: int = 6
    rnn_dim = 1
    network_module_path: str = "net_util.model.sac_rnn"

@register_policy
class SACRNN(PolicyBase):
    """Replay buffer must yield (obs, act, rew, next_obs, done) minibatches."""
    def __init__(self, cmd_cls, cfg: SACRNN_Config, rollout_buffer: ReplayBuffer | None = None, device: str | None = None, state_transform_dict = None):
