...
from net_util.model.sac_rnn_belief_seq import Network
...
class SACRNNBeliefSeq(PolicyBase):
    ...
    def __init__(self, cmd_cls: Any, cfg: SACRNNBeliefSeq_Config, rollout_buffer: RNNPriReplayBuffer2 | None = None, device: str | None = None, state_transform_dict: dict | None = None):
        ...
        self.net: Network = net_class(cfg.obs_dim, cfg.act_dim, belief_dim=cfg.belief_dim).to(self.device)  # type: ignore
    ...
    def _critic_loss(
        ...
        backup_q_TB1, target_q = self.net.target_backup_seq(
            nxt_TBD,
            rew_TB1,
            done_TB1,
            self.cfg.gamma,
        ...
        diff = th.stack([q1_TB1, q2_TB1], dim=0) - backup_q_TB1
        ...
        if self._global_step % self.cfg.log_interval == 0:
            ...
            with th.no_grad():
                ...
                diff_std = d_flat.std(unbiased=False)
                tgt_std2 = tgt_flat.std(unbiased=False)
                std_ratio = diff_std / (tgt_std2 + 1e-12)
                

            # Add target_q / diff stats into cdl_stats for logging
            cdl_stats = {
                "debug/tgt_mean": tgt_mean,
                "debug/tgt_std": tgt_std2,
                "debug/tgt_min": tgt_min,
                "debug/tgt_max": tgt_max,
                "debug/diff_std": diff_std,
                "debug/std_ratio": std_ratio,
            }
