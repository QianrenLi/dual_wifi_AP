...
@register_policy_cfg
@dataclass
class SACRNNBeliefSeqDistV4_Config:
    # Core RL Parameters
    batch_size: int = 256
...
@register_policy
class SACRNNBeliefSeqDistV4(PolicyBase):
    """Refactored Soft Actor-Critic (SAC) with RNN and Learned Belief State."""

    def __init__(self, cmd_cls: Any, cfg: SACRNNBeliefSeqDistV4_Config, rollout_buffer: RNNPriReplayBuffer2 | None = None, device: str | None = None, state_transform_dict: dict | None = None, reward_cfg: Optional[dict] = None):
    ...
    def _probe_z_ablation_feature(
        ...
        z_BH, _ = self.net.belief_encode(obs_TBD, b_h=h_burn)
    ...
    def train_per_epoch(
        ...
        with th.no_grad():
            ...
            z_burn, h_burn = self.net.belief_encode(obs_burn) 
        ...
        ## Belief
        z_BH, _ = self.net.belief_encode(obs_train, b_h=h_burn)
        y_hat_B1 = self.net.belief_decode( z_BH )
        
        b_loss, belief_stats = self._belief_loss( y_hat_B1, interference.unsqueeze(-1), epoch )
    ...
    def tf_act(self, obs_vec: list[float], is_evaluate: bool = False, reset_hidden: bool = False) -> Dict[str, np.ndarray]:
        ...
        z_BH, _belief_h = self.net.belief_encode(obs, self._belief_h, is_evaluate=is_evaluate)
        y_hat_B1 = self.net.belief_decode(z_BH)
