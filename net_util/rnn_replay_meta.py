...
class RNNPriReplayMeta:
    ...
    def add_episode(self, states, actions, rewards, network_output,
        ...
        for i in range(num_episodes):
            ...
            if i == 0:
                # Store initial state of the first episode
                if len(self.initial_states) < self.max_init_states:
                    self.initial_states.append(obs_np[start].copy())
                else:
                    idx = random.randint(0, self.max_init_states - 1)
                    self.initial_states[idx] = obs_np[start].copy()
    ...
    def sample_initial_states(
        ...
        if not self.initial_states:
            return None
        dev = th.device(device) if device is not None else self.device
        n = min(batch_size, len(self.initial_states))
        idx = np.random.choice(len(self.initial_states), size=n, replace=False)
        states_np = np.stack([self.initial_states[i] for i in idx], axis=0).astype(np.float32)
        states_t = th.from_numpy(states_np).to(dev, non_blocking=True)
        return states_t
